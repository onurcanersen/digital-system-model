...
from model.status import AcquisitionStatus, GenerateUnitStatus
from model.validation import ValidationError
from use_cases.clone_software_units import CloneSoftwareUnitsUseCase, CloneUnitResult
...
class RunWorkflowUseCase:
    ...
    def __init__(
        self,
        clone_uc: CloneSoftwareUnitsUseCase,
        generate_uc_factory: Callable[[Path], GenerateModelSetupDataUseCase],
    ):
        self._clone_uc = clone_uc
        self._generate_uc_factory = generate_uc_factory
    ...
    def execute(
        ...
        clone_results = self._clone_uc.execute(
            selection_dir,
        ...
        data = self._generate_uc_factory(selection_dir).execute(
            selection_dir,
            output_path,
            project_id=project_id,
            platform_id=platform_id,
