...
already present under `dest_root` (produced by CloneSoftwareUnitsUseCase
...
class GenerateModelSetupDataUseCase:
    ...
    def execute(
        ...
        extracted_topics = self._analyze_uc.execute(inventory, dest_root)

        validation_errors = self._validate_uc.execute([*acquired_files, *extracted_topics], context)
