...
def _runner(components: Components) -> FakeTaskRunner:
    ...
    return FakeTaskRunner(
        run=lambda task_id, **ids: components.workflow().execute(
...
def test_runs_are_isolated_per_task_id_and_never_reuse(tmp_path: Path):
    ...
    first_dir = Path(client.get(f"/api/tasks/{first['task_id']}").get_json()["result"]["workspace"])
    second_dir = Path(client.get(f"/api/tasks/{second['task_id']}").get_json()["result"]["workspace"])
    assert first_dir != second_dir

    # A new run clones again instead of reusing the previous run's checkout.
    second_result = client.get(f"/api/tasks/{second['task_id']}").get_json()["result"]
