...
def _status(state, result=None, task_id="t-1"):
    ...
    with mock.patch("adapters.celery_task_runner.AsyncResult") as async_result_cls:
...
def test_submit_run_delegates_to_celery_delay():
    runner = CeleryTaskRunner()
    with mock.patch("adapters.celery_task_runner.run_msd_workflow") as task:
        task.delay.return_value = _FakeAsyncResult(states.PENDING, task_id="abc")
        assert runner.submit_run("proj-1", "plat-1", "1.0.0") == "abc"
    task.delay.assert_called_once_with("proj-1", "plat-1", "1.0.0")
