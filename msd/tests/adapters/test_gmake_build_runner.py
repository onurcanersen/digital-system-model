"""Tests for gmake_build_runner.py — find_valid_makefile is pure filesystem logic
...
from msd.adapters.source_code.gmake_build_runner import (
    GmakeBuildRunner,
...
def test_run_regenerate_code_reports_missing_gmake_gracefully(monkeypatch, tmp_path):
    monkeypatch.setattr("msd.adapters.source_code.gmake_build_runner.subprocess.run", _raise_file_not_found)
...
# --- GmakeBuildRunner (IBuildRunner adapter) ----------------------------------


def test_ensure_available_raises_when_gmake_missing(monkeypatch):
    monkeypatch.setattr("msd.adapters.source_code.gmake_build_runner.check_gmake_available", lambda: False)

    with pytest.raises(RuntimeError, match="gmake is not available but build execution was requested"):
        GmakeBuildRunner(PATTERNS).ensure_available()


def test_ensure_available_passes_when_gmake_present(monkeypatch):
    monkeypatch.setattr("msd.adapters.source_code.gmake_build_runner.check_gmake_available", lambda: True)
...
def test_regenerate_code_is_no_op_without_valid_makefile(monkeypatch, tmp_path):
    recorded = []
    monkeypatch.setattr("msd.adapters.source_code.gmake_build_runner.run_regenerate_code",
                        lambda path, timeout=None: recorded.append(path) or (True, ""))
    unit_dir = tmp_path / "nav_app"
    unit_dir.mkdir()
    (unit_dir / "Makefile").write_text(".PHONY: build\n", encoding="utf-8")

    GmakeBuildRunner(PATTERNS).regenerate_code(unit_dir)
