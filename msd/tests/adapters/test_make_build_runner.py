...
from msd.adapters.source_code.make_build_runner import (
    MakeBuildRunner,
...
def test_run_regenerate_code_reports_missing_gmake_gracefully(monkeypatch, tmp_path):
    monkeypatch.setattr("msd.adapters.source_code.make_build_runner.subprocess.run", _raise_file_not_found)
...
def test_ensure_available_raises_when_gmake_missing(monkeypatch):
    monkeypatch.setattr("msd.adapters.source_code.make_build_runner.check_gmake_available", lambda: False)
...
def test_ensure_available_passes_when_gmake_present(monkeypatch):
    monkeypatch.setattr("msd.adapters.source_code.make_build_runner.check_gmake_available", lambda: True)
...
def test_regenerate_code_is_no_op_without_valid_makefile(monkeypatch, tmp_path):
    recorded = []
    monkeypatch.setattr("msd.adapters.source_code.make_build_runner.run_regenerate_code",
